...
vector<Ellipse> get_ellipse_array(std::vector<Obstacle> &seg_obs, const grid_map::GridMap &localmap_obs)
{
  ...
  for (const auto &obs : seg_obs)
  {
    ...
    // added: sanity check
    if(std::isinf(ellipse.theta) || std::isnan(ellipse.theta) || 
       std::isinf(ellipse.cx) || std::isnan(ellipse.cx) ||
       std::isinf(ellipse.cy) || std::isnan(ellipse.cy) ||
       std::isinf(ellipse.semimajor) || std::isnan(ellipse.semimajor) || 
       std::isinf(ellipse.semiminor) || std::isnan(ellipse.semiminor)){
        ROS_WARN("inf or nan exists in get_ellipse_array of ellipse.hpp, skiped");
        continue;
    }
